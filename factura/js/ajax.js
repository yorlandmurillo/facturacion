...
function ventana(ventana,nombre){
	var buscar
   buscar=window.open(ventana,nombre,'width=600,height=400,top='+((screen.height/2)-(180.5))+',left='+((screen.width/2)-(310.5))+',toolbar=no,scrollbars=yes,resizable=no,menubar=no,status=no,directories=no,location=no,dependent=yes,alwaysRaised=yes')

}

function revisatalonario()
{
	//alert(document.ventas.cliente.value);
	if(vacio(document.talonario.codfacturamanual.value) == false && document.talonario.fecfacturamanual.value == '0000-00-00') {
			mostrardiv();
			return false
	}
}

function muestraviso() {
	alert("Los datos del talonario fueron cambiados");
	window.location.reload();
}


function mostrardiv() {
	div = document.getElementById('talonariovacio');
	div.style.display ='';
}

function validarncontrol(ncontrol)
{
	var numcontrol=ncontrol.value;
	if(numcontrol.length <6)
	{
		alert("Número de control muy corto");
	}
	else if(numcontrol.lastIndexOf("'")!=-1)
	{
		alert("Caracter no válido en el Número de control (')");
...
function validarcodfacturamanual(codfacturamanual)
{
	...
	else if(factura.lastIndexOf("'")!=-1)
	{
		alert("Caracter no válido en codigo de factura manual (')");
...
function aceptarfactura(){
	...
	pagos="";
	facturatalonario="";
	fecha_s=fecfacturamanual.split('-');
	fecha_n=fecha_s[2]+"/"+fecha_s[1]+"/"+fecha_s[0];
	i=0;
	//alert("codf="+codf+"\ncliente="+cliente+"\nvend="+vend+"\ntotal="+total+"\nfecfacturamanual="+fecfacturamanual+"\ncodfacturamanual="+codfacturamanual+"\nncontrol="+ncontrol);
	...
	if(document.ventas.otromonto.value=="")omoneda=0;
	...
	montodepago  = new Array();
	montodepago[0] = efec;
	montodepago[1] = tdb;
	montodepago[3] = tdc;
	montodepago[4] = bl;
	montodepago[5] = esp;
	montodepago[6] = mtocheque;
	montodepago[7] = omoneda;
	montodepago[8] = cesta;
	
	formadepago  = new Array();

	formadepago[0] = "Efectivo";
	formadepago[1] = "Tarjeta de Debito";
	formadepago[3] = "Tarjeta de Credito";
	formadepago[4] = "Bono Libro";
	formadepago[5] = "Bono Libro";
	formadepago[6] = "Cheque";
	formadepago[7] = "Otra monedas";
	formadepago[8] = "Cesta Tickect";
	
	for(i = 0; i<montodepago.length; i++)
	{
		if(montodepago[i]>0)
		{
			pagos=pagos+formadepago[i]+": "+montodepago[i]+"\n";
		}
	}
	
	

	if(total==0){alert("La Factura esta en Cero");return false}
		montofactura=Math.round(total * 100) / 100;
		montopagado=Math.round(totalp * 100) / 100;
		montocambio=Math.round(cambio * 100) / 100;
		
	if(tipofactura==3)
	{
		facturatalonario="Asegurese de haber cargado todos los articulos de factura manual antes de procesarla\n\n                             FACTURA MANUAL\nFecha de la factura: "+fecha_n+"\n\n";
		if(fecfacturamanual=="0000-00-00" || codfacturamanual=="" || ncontrol=="")
		{
			alert("Faltan campos de la factura manual");return false;
		}
	...
	{
		msg=alert("No ha especificado una forma de pago");
		return false;
	}
	else
	{
		msg=confirm(facturatalonario+"Forma de pago: \n"+pagos+"\nMonto de la factura: "+montofactura+"\nMonto pagado: "+montopagado+"\nCambio: "+montocambio+"\n\nVa a procesar una factura \n¿Esta Seguro?");
	}
				...
				if(totalp>total || totalp==total)
				{
					...
					ajax.onreadystatechange=function() 
					{
						if (ajax.readyState==4) {
							//mostrar resultados en esta capa
							divResultado.innerHTML = ajax.responseText
							document.ventas.cantidad.disabled="true";
							document.ventas.codproducto.disabled="true";
							document.ventas.additemf.style="display:none";
							document.ventas.titulos.style="display:none";
							document.ventas.Aceptar.style="display:none";
							document.ventas.Cancelar.style="display:none";
							document.ventas.montoefectivo.disabled="true";
							document.ventas.montotdb.disabled="true";
							document.ventas.montotdc.disabled="true";
							document.ventas.montobl.disabled="true";
							document.ventas.montoesp.disabled="true";
							document.ventas.otromonto.disabled="true";
							document.ventas.montocheque.disabled="true";
							document.ventas.nrocheque.disabled="true";
							document.ventas.nrocuenta.disabled="true";
							//window.close(ventas);
							//llamar a funcion para limpiar los inputs
							if(tipofactura==3)
							{
								verfactura(codfacturamanual,suc,vend);
							}
							else
							{
...
function actualizatalonario(){
	//valores de los inputs
	codf=document.talonario.codfactura.value;
	suc=document.talonario.sucursal.value;
	fecfacturamanual=document.talonario.fecfacturamanual.value;
	codfacturamanual=document.talonario.codfacturamanual.value;
	ncontrol=document.talonario.ncontrol.value;
	tipo=document.talonario.tipo.value;
	pagina=document.talonario.pagina.value;
	cliente=document.talonario.cliente.value;
	fecha_act=document.talonario.fecha_actual.value;
	
	
	if(fecfacturamanual!="" & codfacturamanual!="" & ncontrol!="")
	{
		if(fecfacturamanual<=fecha_act)
		{
			if(fecfacturamanual!='0000-00-00')
			{
				if(codfacturamanual.length >5)
				{
					if(ncontrol.length >5)
					{
						//alert("codf="+codf+"&suc="+suc+"&fecfacturamanual="+fecfacturamanual+"&codfacturamanual="+codfacturamanual+"&ncontrol="+ncontrol+"&cliente="+cliente+"&tipo="+tipo+"&pagina="+pagina);
						//instanciamos el objetoAjax
						ajax=objetoAjax();
						//uso del medotod POST
						//archivo que realizará la operacions
						ajax.open("POST","funciones/actualiza_talonario.php",true);
						ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
						//enviando los valores
						ajax.send("codf="+codf+"&suc="+suc+"&fecfacturamanual="+fecfacturamanual+"&codfacturamanual="+codfacturamanual+"&ncontrol="+ncontrol+"&cliente="+cliente+"&tipo="+tipo+"&pagina="+pagina)
					}
					else alert("Numero de control muy corto");return false;
				}
				alert("Codigo de factura manual muy corto");return false;
			}
			else alert("No se ha ingresado la fecha de la factura del talonario");return false;
		}
		else alert("La fecha del factura manual no puede ser mayor que la del sistema");return false;
		
	}else alert("Faltan campos de la factura manual");return false;

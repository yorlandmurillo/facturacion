...
class factura extends  manejadordb{
var $year;
var $month;
var $day;
var $hour;
...
function codigo($vendedor,$sucursal,$cliente,$tipo){
//die("-*->tipo=".$tipo);
    $year=date('Y');
	$month=date('m');
	$day=date('d');
...
//Selecciono el correlativo de la factura actual
	...
	$codfacturaactual=$cadena.$cod2;
	
	if($cod=="")$cod=1;else $cod+=1;
	$codfactura=$cadena.$cod;
	...
	{
		if($tipo==n)
			$codigof->query("INSERT INTO tbl_facturas (cod_factura,fecha_factura,cod_cliente,vendedor,sucursal,correlativo)VALUES('$codfactura',NOW(),'".$cliente."',$vendedor,$sucursal,$cod);");
		elseif($tipo==m)
			$codigof->query("INSERT INTO tbl_facturas (cod_factura,fecha_factura,cod_cliente,vendedor,sucursal,correlativo)VALUES('$codfactura','0000-00-00 00:00:00','".$cliente."',$vendedor,$sucursal,$cod);");
		return $codfactura;
...
/* Funcion para efectuar el cierre diario */
	...
	if($factura->query("update tbl_cierre set total_clientes=$num,total_ejemplares=$item,total_credito=$tdc,total_debito=$tdb,total_efectivo=$efectivo,total_bonolibro=$bl,cant_cheques=$nc,total_cheques=$cheque,total_descuentos=$descuento,total_especiales=$especial,total_cestatikets=$cesta,total_omoneda=$omoneda,factura_inicial='$fi',factura_final='$ff',cerrado_por=$vendedor,fecha_cierre='$fechaa', estatus=7 where fecha='$fechac' and cod_sucursal=$suc and estatus=6 ")==true){
		return true;
		}else return false;
	}else false;	
	}
...
/* Funcion para efectuar el cierre automatico del dia */
	...
	if($num==0)$factura->query("delete from tbl_cierre where fecha='$date' and cod_sucursal=$suc and estatus=6 ;");
	...
	if($factura->query("update tbl_cierre set total_clientes=$num,total_ejemplares=$item,total_credito=$tdc,total_debito=$tdb,total_efectivo=$efectivo,total_bonolibro=$bl,cant_cheques=$nc,total_cheques=$cheque,total_descuentos=$descuento,total_especiales=$especial,total_cestatikets=$cesta,total_omoneda=$omoneda,factura_inicial='$fi',factura_final='$ff',estatus=7,fecha_cierre='$fechaa' where fecha='$date' and cod_sucursal=$suc and estatus=6 ;")==true){
...
/**************************************/
	...
	if($num2=mysql_num_rows($result2)==0){
		...
		$factura->query("delete from tbl_facturas where estatus_factura=0 and fecha_factura<'$fechac'");
	...
	function cierresesion($vend,$suc){
    $year=date(Y);
	$month=date(m);
	$day=date(d);
	$hour=date(H)-1;
	...
	if($factura->query("update tbl_sesiones set sesiones_us_ex='$fechaa',estatus=19 where id_user=$vend and id_sucursal=$suc and  estatus=9;")==true){
	  if($factura->query("update log_sesiones set sesiones_us_ex='$fechaa',expirada=10,estatus=19 where id_user=$vend and id_sucursal=$suc and expirada=9 and estatus=9;")==true){
		return true;
	  }
	}else return false;
	}

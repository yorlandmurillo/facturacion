...
$editorial_enviada="";
$isbn_enviado="";
$codigodebarra_enviado="";
$tomo_enviado="";
$presentacion_enviada="";
$precio_unitario_enviado="";
$cantidad_enviada="";
...
if($_POST['submit'])
{
	...
	if (is_dir($ruta))
	{
		...
		{
			//$nota_entrega=substr($archivo, 0, 12);
			
			$nota_entrega = substr($archivo,0,-4);
			...
			{	
			  ?>
				<a href="lee_notas.php?arch=<? echo $archivo?>&id_suc=<? echo $id_sucursal?>&carga=<? echo $carganota?>"><? echo $archivo?></a><br>
				<?
			}
		}
		closedir($directorio);
	...
	?><br>
		<div align='center'><a href="lee_notas.php">Regresar al inicio</a></div><br>
		<?
}
elseif($_GET[arch]!='')
{
	$id_sucursal=$_GET[id_suc];
	$nota_entrega=substr($_GET[arch], 0, -4);
	    ...
	    <TD width='12' align="center" bgcolor="#990000"><font color='white'><b>Cod. Barra</b></font></TD> 
		  ...
		  <TD width='48' align="center" bgcolor="#990000"><font color='white'><b>Cant Entreg</b></font></TD> 
			...
			{ 
					...
					if(strpos($contenido, '#')!=false)
					{
						...
						if($nf_libro>0)
						{
							...
							while($row = mysql_fetch_row($status_libro))
							{
								...
								?>
								<!--<tr>
								<td><input type="text" name="codlibro[<?echo $j?>]" value="<? echo $cod_libro; ?>" readonly></td>
								<td><input type="text" name="titulo[<?echo $j;?>]" value="<?echo $titulo;?>" readonly></td>
								<td><input type="text" name="autor[<?echo $j;?>]" value="<?echo $autor_name;?>" readonly></td>
								<td><input type="text" name="editorial[<?echo $j;?>]" value="<?echo $editorial;?>" readonly></td>
								<td><input type="text" name="isbn[<?echo $j;?>]" value="<?echo $isbn;?>" readonly></td>
								<td><input type="text" name="cod_Barra[<?echo $j;?>]" value="<?echo $codigodebarra;?>" readonly></td>
								<td><input type="text" name="tomo[<?echo $j;?>]" value="<?echo $tomo;?>" readonly></td>
								<td><input type="text" name="presentacion[<?echo $j;?>]" value="<?echo $presentacion;?>" readonly></td>
								<td><input type="text" name="precio_unitario[<?echo $j;?>]" value="<?echo $precio_unitario;?>" readonly></td>
								<td><input type="text" name="cantidad[<?echo $j;?>]" value="<?echo $cantidad;?>" readonly></td>
								</tr>-->
								<?
								if($j==0)
								{
									$codigo_enviado=$cod_libro;
									$titulo_enviado=$titulo;
									$autor_enviado=$autor_name;
									$editorial_enviada=$editorial;
									$isbn_enviado=$isbn;
									$codigodebarra_enviado=$codigodebarra;
									$tomo_enviado=$tomo;
									$presentacion_enviada=$presentacion;
									$precio_unitario_enviado=$precio_unitario;
									$cantidad_enviada=$cantidad;
								}
								else
								{
									$codigo_enviado=$codigo_enviado."*".$cod_libro;
									$titulo_enviado=$titulo_enviado."*".$titulo;
									$autor_enviado=$autor_enviado."*".$autor_name;
									$editorial_enviada=$editorial_enviada."*".$editorial;
									$isbn_enviado=$isbn_enviado."*".$isbn;
									$codigodebarra_enviado=$codigodebarra_enviado."*".$codigodebarra;
									$tomo_enviado=$tomo_enviado."*".$tomo;
									$presentacion_enviada=$presentacion_enviada."*".$presentacion;
									$precio_unitario_enviado=$precio_unitario_enviado."*".$precio_unitario;
									$cantidad_enviada=$cantidad_enviada."*".$cantidad;
								}
								$j++;
							}//echo "-*->".$j."<br>";
				...
				?>
				<input type="hidden" name="filas" value="<?echo $j?>" readonly>
				<input type="hidden" name="codigo_enviado" value="<?echo $codigo_enviado;?>">
				<input type="hidden" name="titulo_enviado" value="<?echo $titulo_enviado;?>">
				<input type="hidden" name="autor_enviado" value="<?echo $autor_enviado;?>">
				<input type="hidden" name="editorial_enviada" value="<?echo $editorial_enviada;?>">
				<input type="hidden" name="isbn_enviado" value="<?echo $isbn_enviado;?>">
				<input type="hidden" name="codigodebarra_enviado" value="<?echo $codigodebarra_enviado;?>">
				<input type="hidden" name="tomo_enviado" value="<?echo $tomo_enviado;?>">
				<input type="hidden" name="presentacion_enviada" value="<?echo $presentacion_enviada;?>">
				<input type="hidden" name="precio_unitario_enviado" value="<?echo $precio_unitario_enviado;?>">
				<input type="hidden" name="cantidad_enviada" value="<?echo $cantidad_enviada;?>">
...
{
	?>
		<b><div align='center'><font size='5'>APLICACION PARA LA CARGA OFFLINE DE LAS NOTAS DE ENTREGA<br>
		SELECCIONE UNA LIBRERIA</font></div></b>
		<br><br><br>
	<?
	...
	while($row = mysql_fetch_row($result))
	{
		...
		<div align='center'>
		 <form action="" method="post">  
		  ...
		  {
		   ?>
			   <option value="<?php echo $id[$i]."-".$nombresucursal[$i]; ?>"><?php echo $nombresucursal[$i]; ?></option>
			   <?php
		   }
		  ?>

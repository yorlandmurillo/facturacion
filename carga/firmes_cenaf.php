...
{
var pagina="firmes_cenaf.php?codigo="+codigo
...
function abreventana() {
 popupWin = window.open('listado_cenaf.php',"V1","menubar=no,titlebar=no,toolbar=no,location=no,status=no,directories=no,fullscreen") 
 }
...
//	$descripcion=$_POST['descripcion'];
	...
	if ($myrow= mysql_fetch_array($result_bienes)) 
	{
		...
		do 
		{
			...
			$editorial=$myrow[3];
			if(substr($cod_producto,0,4)=='4444'||substr($cod_producto,0,4)=='0404')
			{
			...
			<td align='center'><b><a href="firmes_cenaf.php?codigol=<? echo $myrow[0]?>&buscar=<? echo $buscar2?>">EDITAR</a></b></td>
			<?
				if($firme==0)
				{
					?>
					<td align='center'><b><a href="firmes_cenaf.php?codigo2=<? echo $myrow[0]?>"><font color='479211'>CREAR FIRME</font></a></b></td>
	...
	?>
	<br><br>
	<div align="center"><font size='5'><a href="firmes_cenaf.php">Regresa al principio</a></font></div><br>
<?
}
...
{ 
	...
	if ($rowautor= mysql_fetch_array($result_autor)) 
	{
		...
		{
		?>
			<a href="firmes_cenaf.php?id_autor=<? echo $rowautor["id_autor"]?>"><? echo $rowautor["id_autor"]?></a>
			<?
...
{ 
	...
	{	
		...
		?>
			<script >
				location.href="firmes_cenaf.php?id_autor=<? echo $id_autor?>";
			</script>
...
{ 
	...
	?>
		<font size=4><b><div align='center'><font size='5'>CREA EL LIBRO FIRME<br>
		 <form action="firmes_cenaf.php"  method="post">
...
{ 
	...
	$act_cod_libro="update tbl_cod_cenaf set ocupado=1 where cod_libro='$codigo'";
	...
	{ 
		...
		//	die($sql_act_inventario);
			...
			if(mysql_query($sql_actualiza))
			{
				?>
				<script >
					location.href="firmes_cenaf.php";
				</script>
			<?
			}	
		}
...
<div align='center'><font  size='5'color='red'><b>CENAF<b></font></div><BR>

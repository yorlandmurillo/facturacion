...
if ($cod_entrega!= '')
{
	...
	if($row = mysql_fetch_array($result))
	{
		...
		///////////////////////////////// alberto	
		$resultFac = @mysql_query("select * FROM fdvl_operaciones WHERE REC_NUMNOT ='$nota_entrega'",$link); 
	...
	if ($cod_entrega != "")
	{
		...
		while($row = mysql_fetch_array($result))
		{
			...
			printf("<td>%s</td> ", isset($row[2])?utf8_decode($row[2]):' - '); // aut_nombre
						printf("<td>%s</td> ", isset($row[3])?utf8_decode($row[3]):' - '); // aut_nombre
			if ($row[4] == 0) 
			{
				$row[4] = '---';
				printf("<td>%s</td> ", isset($row[4])?$row[4]:' - '); // lib_numedit
			}
			else
			{
				printf("<td>%s</td> ", isset($row[4])?$row[4]:' - '); // lib_numedit
			...
			{
				$row[5] = '---';
				printf("<td>%s</td> ", isset($row[5])?$row[5]:' - '); // lib_numedit
			}
			else
			{
				printf("<td>%s</td> ", isset($row[5])?$row[5]:' - '); // lib_numedit
			...
			{
				$row[7] = 'UNICA';
				printf("<td>%s</td> ", isset($row[7])?$row[7]:' - '); // lib_numedit
			}
			...
			printf("<td>%s</td> ", isset($row[8])?number_format($row[8],2,',','.'):'0,00'); // ped_precio
			printf("<td>%s</td> ", isset($row[9])?$row[9]:'0'); // ped_cantid
			$row[11] = $row[9];
			...
			printf("<td>%s</td> ", number_format($row[10]+(isset($cantEnt)?$cantEnt:'0'),2,',','.'));
			echo("</tr></tbody>");
		}
		mysql_free_result($result);
		mysql_close($link);

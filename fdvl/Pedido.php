...
if ($cliente != ""){
	...
	$strQry="select clt_direc, clt_rif, clt_telef, clt_contac,UPPER(clt_nombre) from inv_cliente where clt_codcli = ". $usuario;
	$result=mysql_query($strQry,$link);
	if($row = mysql_fetch_array($result)){
  		...
  		<tr>
			...
			<td colspan="3"><select class="selectPedido" id="cliente" name="cliente" onchange="frmPedido.submit();">
                	<option value="0" <?php if($cliente==""){echo("selected");} ?>>--Seleccione--</option>
									...
									$result=mysql_query("select clt_codcli, rtrim(clt_nombre) from inv_cliente WHERE clt_codcli = '$usuario'",$link);
...
<?php
									$link=Conectarse();
									$result=mysql_query("select trs_codtrs, rtrim(trs_descrip) from inv_transac where trs_codtrs='2002' order by trs_codtrs",$link);
									while($row = mysql_fetch_array($result)) {
...
<?php
									$link=Conectarse();
									$result=mysql_query("select con_codigo, UPPER(rtrim(con_descri)) from inv_condic where con_codigo = '0003' ",$link);
									while($row = mysql_fetch_array($result)) {
										if($row[0]==$pago){
											printf("<option value='%s' selected>%s - %s</option> ", $row[0],$row[0], utf8_decode($row[1]));
										}else{
...
<div id="FormaModOperativo">
	...
	<table>
		<tr>
			...
			<INPUT TYPE="button" VALUE="Buscar"  onclick="buscaProveedorP(fieldsFormatForm,this.form,1);"  class='noimprimir'>
			</td>
		</tr>
		<tr><td colspan="2"></td></tr>
		<tr>
			<td colspan="2" align="center">
				<INPUT TYPE="button" VALUE="Buscar Articulo"  onclick="buscaArticulo(fieldsFormatForm2,editorial.value);" class='noimprimir'>
			</td>
		</tr>
	</table>
</div>
...
<span class="noimprimir2">Detalle de los Articulos (Libros) a Solicitar</span>
...
<div id='estTabTotal'>
	<TABLE id="tabTotal" name="tabTotal">
		...
		<TR>
			...
			<TD><input type='text' id="desc" name="desc" size='5' readonly="readonly" maxlength='5' value='40,00' onKeyPress="return esNumero(event,'f',this);" onChange='calcularTotalGeneral();'> %</TD>
		</TR>
		<TR>
			<TD></TD>
...
<?php
								$link=Conectarse();
								$result=mysql_query("select ven_codven, rtrim(ven_nombre) from inv_vende where  ven_codven = '0007'",$link);
								while($row = mysql_fetch_array($result)) {
									if($row[0]==$vendedor){
										printf("<option value='%s' selected>%s - %s</option> ", $row[0],$row[0], utf8_decode($row[1]));
									}else{
		...
		<td width='220'><input name="contacClt" type="text" id="contacClt" value="<?php echo($mycliente); ?>" size="30" readonly="readonly"></td>
	</tr>
	<tr>
		<td>Observaciones:</td>

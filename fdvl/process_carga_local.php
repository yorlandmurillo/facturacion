...
for($j=0;$j<count($cod_libro);$j++)
{
	...
	if ($myrow_carga= mysql_fetch_array($resultcarga)) 
	{
			...
			if($cantidad_items > 0)
			{
				...
				$status_nota_operaciones=mysql_query($busca_nota_operaciones,$link2) or die("<br>".mysql_error()); //apuntando a la base de datos local
				...
				if($nf_nota_operaciones==0)
				{
					...
					$result=mysql_query($strQry,$link2); //apuntando a la base de datos local
					...
					mysql_query($sql,$link2);//Debe apuntar a la base de datos remota
					...
					@mysql_query($sql2,$link2); //Debe apuntar a la base datos remota
					echo $sql2."<br><br>";

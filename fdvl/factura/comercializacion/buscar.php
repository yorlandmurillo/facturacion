...
function enviarcodigo(codigo,titulo,autor,editorial,costo){
    window.opener.document.forms[0].codigo.value = codigo;
    window.opener.document.forms[0].titulo.value = titulo;
    window.opener.document.forms[0].autor.value = autor;
    window.opener.document.forms[0].editorial.value = editorial;
    window.opener.document.forms[0].costo.value = costo;
	window.opener.cerrarbuscar();
}
...
$obj=new manejadordb;
 ...
 if( isset($_GET['buscar']) && !empty($_GET['buscar']))  
 {
    ...
    // add field
     $query .= " cod_producto = '".addslashes($_GET['buscar'])."' OR";
     $query .= " descripcion = '".addslashes($_GET['buscar'])."' OR";
     $query .= " isbn = '".addslashes($_GET['buscar'])."' OR";
...
$query .= " order by ".$_GET['order']." $ord ";
 ...
 // ************* end of buscar *****************/  
     ...
     echo "<table class=\"tabla\" border=\"0\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">\n";     
         ...
         echo "<th class=\"celdal\"><a href=".$_SERVER['PHP_SELF']."?order=descripcion"."&"."forma=".$forma.">".ucfirst("Descripci&oacute;n")."</th>\n";
         echo "<th class=\"celdal\"><a href=".$_SERVER['PHP_SELF']."?order=autor"."&"."forma=".$forma.">".ucfirst("Autor")."</th>\n";
         echo "<th class=\"celdal\"><a href=".$_SERVER['PHP_SELF']."?order=isbn"."&"."forma=".$forma.">".ucfirst("Isbn")."</th>\n";
         echo "<th class=\"celdal\"><a href=".$_SERVER['PHP_SELF']."?order=costo"."&"."forma=".$forma.">".ucfirst("Precio")."</th>\n";
          $i = 0;
     ...
     {         
        ...
        echo "<tr class=\"selecionada\" onclick=\"enviarcodigo('".$row["cod_producto"]."','".$row["descripcion"]."','".$row["autor"]."','".$row["editorial"]."','".$obj->setcosto_remoto($row["cod_producto"])."')\">\n";
         echo "<td>".$row["cod_producto"]."</td>\n";
         echo "<td>".substr($row["descripcion"],0,30).$pts."</td>\n";
         echo "<td>".substr($row["autor"],0,10).$pts."</td>\n";
         echo "<td>".$row["isbn"]."</td>\n";

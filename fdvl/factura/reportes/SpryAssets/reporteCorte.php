...
$fch_creacion2 = isset($_POST['fch_creacion2'])?cambiaf_a_mysql($_POST['fch_creacion2']):'0000-00-00';
...
$query_Recordset1=
"select NOT_CANTID, NOT_PRECIO, (NOT_CANTID * NOT_PRECIO) AS TOTAL, inv_notaed.NOT_NUMNOT, NOT_CODART, NOT_NUMPED, ven_nombre,clt_nombre, NOT_FCHNOT, clt_direc, clt_rif,clt_telef,NOT_CANTID,NOT_PRECIO,aut_nombre,lib_descri, trs_descrip,ped_observ,clt_codcli, ped_pordes, prv_nombre,inv_notaec.not_user, con_descri, trs_descrip,inv_pedidc.con_codigo,trs_codtrs
...
jOIN inv_condic
on inv_condic.con_codigo = inv_pedidc.con_codigo
WHERE inv_notaed.NOT_NUMNOT ='$nota_entrega'
AND NOT_CANTID <> 0
ORDER BY inv_libros.lib_descri,inv_libros.lib_codart ";
...
<table width="100%" border="0">
  ...
  <table width="729">
    ...
    <tr>
      <td class="TextFiel">Desde:</td>
      <td><img src="images/img.gif" id="f_trigger_c"
										style="cursor: pointer; border: 1px solid #990000;"
										title="Date selector"
										onmouseover="this.style.background='990000';"
										onmouseout="this.style.background=''" />
        <input size="10" type="text" id="fch_creacion" name="fch_creacion" value="<? echo cambiaf_a_normal($fch_creacion);?>" readonly="readonly" />
        
        
        
        </td>
      <td>&nbsp;</td>
      <td><img src="images/img.gif" alt="" name="f_trigger_c2" id="f_trigger_c2"
										style="cursor: pointer; border: 1px solid #990000;"
										title="Date selector"
										onmouseover="this.style.background='990000';"
										onmouseout="this.style.background=''" />
        <input size="10" type="text" id="fch_creacion2" name="fch_creacion2" value="<? echo cambiaf_a_normal($fch_creacion2);?>" readonly="readonly" /></td>
    </tr>
    <tr>
...
?>
    ...
    <tr>
      <td class="TextFiel">Transacción:</td>
      <td colspan="3"><input name="transaccion" type="text" class="inputCodigo2" id="vendedor4" value="<?php echo $row_Recordset1['trs_descrip']; ?>" size="30" readonly="readonly" /></td>
    </tr>
    <tr>
      <td class="TextFiel">Forma de Pago:</td>
      <td colspan="3"><input name="pago" type="text" class="inputCodigo2" id="vendedor3" value="<?php echo $row_Recordset1['con_descri']; ?>" size="30" readonly="readonly" /></td>
    </tr>
    <tr>
...
if ($row_Recordset1['trs_codtrs'] == 2001  or $row_Recordset1['trs_codtrs'] == 2002 )
{ 
}
else
{ 
echo "<script>alert('LA NOTA DE ENTREGA NO ES UNA VENTA EN FIRME O UNA CONSIGNACIÓN.');
document.location=('inventario.php?p=facturaPrivada');
</script>"; 
}
...
<script type="text/javascript">
	...
	Calendar.setup(
	  {
		inputField : "fch_creacion2", // ID of the input field
		ifFormat : "%d/%m/%Y", // the date format
...
<?PHP
function guardar(){
$link = mysql_connect("localhost","distri","DISTRI77dora") or die (mysql_error());  
mysql_select_db("distri_fdvl",$link);
...
$result = @mysql_query("
select NOT_CANTID, NOT_PRECIO, (NOT_CANTID * NOT_PRECIO) AS TOTAL, inv_notaed.NOT_NUMNOT, NOT_CODART, NOT_NUMPED, ven_nombre,clt_nombre, NOT_FCHNOT, clt_direc, clt_rif,clt_telef,NOT_CANTID,NOT_PRECIO,aut_nombre,lib_descri, ped_observ,clt_codcli, ped_pordes, prv_nombre,  inv_notaec.not_user, con_descri, trs_descrip,inv_pedidc.con_codigo,trs_codtrs
FROM inv_notaed INNER JOIN inv_notaec
on inv_notaed.NOT_NUMNOT = inv_notaec.NOT_NUMNOT
JOIN inv_pedidc
on inv_notaec.NOT_NUMPED = inv_pedidc.ped_numped
...
//If the current row is the last one, create new page and print column title
    if ($i == $max)
    {
  $sql = "INSERT INTO inv_facturac (fac_fecha, fac_hora,fac_numnot,fac_usuario,fac_mtotal,fac_mtodcto,fac_mtontalg,fac_cantlib,fac_cantitulo,fac_moneda,fac_estatus,fac_fechav,fac_numped,fac_codcli,fac_fondo,fac_vendedor,fac_trans,fac_usuarioNE, observacion, fac_fpago	)
    VALUES
   ('$fecha','$hora','$nota_entrega','$usuario','$totalPagar','$descuento','$pagarFinal','$totalC','$i','$moneda','$estatus','$fec_vencimiento','$cod_pedido','$cliente','$fondo','$vend','$trans','$usuarioN','$nota', '$pago')";

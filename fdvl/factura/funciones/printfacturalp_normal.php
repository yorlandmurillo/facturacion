...
$query="
SELECT tbl_itemfactura.cod_factura AS factura, tbl_facturas.fecha_factura AS fecha,
tbl_facturas.sub_total AS sub_total,
tbl_facturas.mto_iva AS mto_iva,
tbl_facturas.descuento AS descuento,
...
tbl_itemfactura.iva AS iva, 
tbl_cliente.cli_cedula AS cedula, tbl_cliente.cli_nombre AS nombre, 
tbl_usuario.us_login AS vendedor, tbl_sucursal.sucursal AS sucursal,tbl_sucursal.telefono AS telefono 

FROM (((tbl_facturas LEFT JOIN tbl_itemfactura ON (tbl_facturas.sucursal = tbl_itemfactura.sucursal) AND 
(tbl_facturas.cod_factura = tbl_itemfactura.cod_factura)) INNER JOIN tbl_cliente ON tbl_facturas.cod_cliente = tbl_cliente.cli_cedula) 
INNER JOIN tbl_usuario ON tbl_facturas.vendedor = tbl_usuario.id_usuario) INNER JOIN tbl_sucursal ON tbl_facturas.sucursal = tbl_sucursal.id_sucursal 
WHERE (((tbl_facturas.cod_factura)='$fact') AND ((tbl_facturas.sucursal)='$sucursal') AND ((tbl_facturas.estatus_factura)=3))
GROUP BY tbl_itemfactura.cod_producto
";
...
$texto ="Libreria del Sur: ".$oficina."\n"."".$rif."\n"."Vendedor:".$vendedor."\n"."Telefono: ".$telefono."\n"."# Factura Contado: ".$factura."\n"."C.I/RIF: ".$CIRIF."\n"."Cliente: ".$NOMB."\n"."Fecha:	".fecha($fecha)."\n"."\t".$contrib."\n".$div."\n".$head."\n".$div."\n"; 

for ($i=0;$i<sizeof($libros) && sizeof($libros) <= $limite_array;$i++){

	$sumaarray+=($obj->setprecio($libros[$i][5])*$libros[$i][1]);
	$sumaiva+=($obj->setprecio($libros[$i][5]))*$libros[$i][3];
	$sumadcto+=($obj->setprecio($libros[$i][5])*$libros[$i][4])*$libros[$i][1];
...
shell_exec("cat ".$_SERVER['DOCUMENT_ROOT']."facturas_lp/".$factura.$letra."> /dev/lp0");
//shell_exec("rm ".$_SERVER['DOCUMENT_ROOT']."funciones/facturas_lp/".$factura.$letra."");
sleep(10);
}
